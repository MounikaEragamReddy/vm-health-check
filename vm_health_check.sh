...
# filepath: /Users/meraga291/mounika-pws/TASK1/vm-health-check/vm_health_check.sh
...
if [[ $1 == "explain" ]]; then
  ...
  if [[ $health_status == "NOT HEALTHY" ]]; then
    echo "Reason(s):"
    [[ $cpu_usage -gt 60 ]] && echo "- CPU usage exceeds 60% (Current: $cpu_usage%)."
    [[ $memory_usage -gt 60 ]] && echo "- Memory usage exceeds 60% (Current: $memory_usage%)."
    [[ $disk_usage -gt 60 ]] && echo "- Disk usage exceeds 60% (Current: $disk_usage%)."
  fi
fi